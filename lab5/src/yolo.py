...
class YOLOv3Head(nn.Module):
    ...
    def __init__(self, num_classes=20, num_anchors=3):
        ...
        # 根據 tf_efficientnetv2_m.in21k_ft_in1k 的輸出通道數來設定
        # C1 (feat_13) -> 512 channels
        # C2 (feat_26) -> 176 channels
        # C3 (feat_52) -> 80 channels
        
        ################# 針對 tf_efficientnetv2_m 修改後的程式碼 ###################
        ...
        # Upsample for scale 2, 將 256 channels 降為 128
        ...
        # 輸入通道數 = 128 (來自上採樣) + C2 (176 來自骨幹) = 304
        self.scale2_conv = nn.Sequential(
            ConvBlock(304, 128, kernel_size=1),
        ...
        # Upsample for scale 3, 將 128 channels 降為 64
        ...
        # 輸入通道數 = 64 (來自上採樣) + C3 (80 來自骨幹) = 144
        self.scale3_conv = nn.Sequential(
            ConvBlock(144, 64, kernel_size=1),
...
class ODModel(nn.Module):
    ...
    def __init__(self, num_classes=20, num_anchors=3, pretrained=True, nms_thres=0.4, conf_thres=0.5):
        ...
        self.backbone = Backbone(pretrained=pretrained, model_name="tf_efficientnetv2_m.in21k_ft_in1k")
        ###########################################################################
