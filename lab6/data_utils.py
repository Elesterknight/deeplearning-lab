...
def QACollator(batch: List[Dict[str, List[int]]]) -> Dict[str, torch.Tensor]:
    ...
    max_tgt_len = max(len(item["labels"]) for item in batch) if batch[0].get("labels") is not None else 0
    ...
    for item in batch:
        ...
        src = item["input_ids"]
        src_len = len(src)
        padded_src = src + [PAD_ID] * (max_src_len - src_len)
        
        src_tokens.append(padded_src)
        src_lens.append(src_len)
        id_s.append(item["id"])

        if max_tgt_len > 0 and item.get("labels") is not None:
            tgt = item["labels"]
            tgt_len = len(tgt)
            
            padded_tgt = tgt + [LABEL_PAD_ID] * (max_tgt_len - tgt_len)
            tgt_tokens.append(padded_tgt)
            tgt_lens.append(tgt_len)

    return {
        "src": torch.tensor(src_tokens, dtype=torch.long),
        "tgt": torch.tensor(tgt_tokens, dtype=torch.long) if tgt_tokens else None,
        "src_len": torch.tensor(src_lens, dtype=torch.int32),
        "tgt_len": torch.tensor(tgt_lens, dtype=torch.int32) if tgt_lens else None,
        "id": id_s,
    }
